...
dataset = pd.read_csv("../input/sep-dataset-9/20161231_20190313_v1.csv",infer_datetime_format=True,
                      parse_dates = ['DATA'])
...
daily_df.head()
daily_df.tail()
#daily_df = daily_df[daily_df['DATA'] <= '2019-03-10']
...
hldys3 = pd.DataFrame({
  'holiday': 'especial',
  'ds': pd.to_datetime(['2017-12-25','2018-01-01','2018-12-25','2019-01-01','2019-12-25']),
  'lower_window': 0,
  'upper_window': 0,
})
...
hldys4 = pd.DataFrame({
  'holiday': 'alta',
  'ds': pd.to_datetime(['2017-02-24','2017-03-01','2017-03-02','2017-05-13', '2017-06-10', '2017-08-12', '2017-11-24', 
                        '2018-02-09','2018-02-14','2018-02-15','2018-05-12', '2018-06-12', '2018-08-11', '2018-11-23',
                        '2019-03-01','2019-03-06','2019-03-07','2019-05-11','2019-06-12','2019-08-10','2019-11-22']),
  'lower_window': 0,
  'upper_window': 0,
})
...
m = Prophet(yearly_seasonality=False,weekly_seasonality=False,growth = 'linear',
           ...
           changepoint_prior_scale = 10, changepoint_range = 1,
           ).add_seasonality(
...
# a['2019-03-01':'2019-03-13']['yhat'].sum()
...
b = dataset.set_index('DATA')['2019-03-01':'2019-03-13']['TRN_TOTAL'].sum()
c = a['2019-03-14':'2019-03-20']['yhat'].sum()
est = b + c
est
'''
a['2019-03-01':'2019-03-21']['yhat']
a['2019-03-01':'2019-03-13']['y'].sum()
a['2019-03-11':'2019-03-13']['yhat'].sum()

a['2018-10-01':'2018-10-31']['yhat'].sum()
a['2018-01-01':'2018-02-28']['y'].plot()
a['2017-03-01':'2017-02-21']['y']
...
plt.plot(dataset.set_index('DATA')['2019-01-01':'2019-03-10']['TRN_TOTAL'].resample('D').apply(sum), '-b', label='Real')
plt.plot(a['2019-02-01':'2019-02-28']['yhat'],'--r', label='Predict')
ax.set_xlabel('data',fontsize=14)
ax.set_ylabel('qtd_transacoes',fontsize=14)
ax.set_title('projeção fev/19',fontsize=14)
ax.legend()
